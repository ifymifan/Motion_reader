...
PinStatus red[3] = {HIGH,LOW,LOW};
PinStatus green[3] = {LOW, HIGH, LOW};
PinStatus blue[3] = {LOW,LOW,HIGH};
PinStatus none[3] = {LOW,LOW,LOW};
PinStatus* colorArray[4] = {red, green, blue, none};
int colorIndex = 0;
void onPowerChange(){
...
{
    message = "";
    setLedColor(colorArray[colorIndex]);
    ArduinoCloud.update();
    if (!power){return;}
    ...
    check_inference_result(result);
}

void setLedColor(PinStatus RGB[]){
  digitalWrite(LEDR,RGB[0]);
  digitalWrite(LEDG,RGB[1]);
  digitalWrite(LEDB,RGB[2]);
}
...
void check_inference_result(ei_impulse_result_t result){
  int result_index = -1;
  for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
  ...
  if (result_index >=0){
    message = ei_classifier_inferencing_categories[result_index];
    colorIndex = result_index;
    ArduinoCloud.update();
    ei_printf("%s %s\n","You said", ei_classifier_inferencing_categories[result_index]);
  }
}
